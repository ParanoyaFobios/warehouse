...
class KeyCRMProductSerializer(serializers.ModelSerializer):
    ...
    quantity = serializers.IntegerField(source='total_quantity', required=False)
    ...
    def save(self, **kwargs):
        ...
        if category_id:
            category, _ = ProductCategory.objects.get_or_create(
                keycrm_id=category_id,
                defaults={'name': f"Категория {category_id}"}
            )

        external_url = data.pop('thumbnail_url', None)
        
        # Поиск товара по SKU или KeyCRM ID
        sku = data.get('sku')
