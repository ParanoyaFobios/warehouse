...
def get_dialogs_list(current_user):
    ...
    active_messages_query = Message.objects.filter(
        ...
        Q(sender=OuterRef('pk'), recipients=current_user, 
          messagerecipient__user=current_user, messagerecipient__is_deleted=False)
    ...
    return User.objects.exclude(pk=current_user.pk) \
        .select_related('profile') \
        .prefetch_related('groups') \
        .annotate(
            last_msg_date=Subquery(active_messages_query),
            unread_count=Coalesce(Subquery(unread_subquery, output_field=IntegerField()), 0)
        ) \
        .filter(last_msg_date__isnull=False) \
...
class CreateDialogView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        # 1. Привязываем отправителя перед сохранением
        message = form.save(commit=False)
        message.sender = self.request.user
        message.save()
        # 2. Сохраняем связи ManyToMany (получателей)
        form.save_m2m()
        
        # 3. Сохраняем объект в self.object для использования в get_success_url
        self.object = message
        return super().form_valid(form)

    def get_success_url(self):
        # Получаем первого получателя, чтобы перекинуть в чат с ним
        recipient = self.object.recipients.first()
        if recipient:
            return reverse('chat_detail', kwargs={'user_id': recipient.pk})
        return reverse('dialogs')

    def form_invalid(self, form):
        print(form.errors) 
        messages.error(self.request, "Ошибка при создании чата.")
...
class ChatView(LoginRequiredMixin, View):
    ...
    def get(self, request, user_id=None):
        ...
        context['new_chat_form'] = NewMessageForm(user=current_user)
